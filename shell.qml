  ...
  Component.onCompleted: {
    ...
    if (saved && sessions.session_execs.length > 0) {
      for (var i = 0; i < sessions.session_names.length; i++) {
        if (sessions.session_execs[i].toLowerCase().includes(saved.toLowerCase())
            || sessions.session_names[i].toLowerCase().includes(saved.toLowerCase())) {
          ...
          Rectangle {
            ...
            MouseArea {
              ...
              onClicked: {
                ...
                Settings.setLastSessionId(ident || sessions.current_session_name)
              }
            }
          }
        }
